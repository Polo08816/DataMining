...
autompg <- read.csv(file = "C:/Users/maryjoyce/Documents/Towson/Spring 2016/COSC 757/Assignments/Assignment1/UCI data/auto-mpg.data-original.txt", header = FALSE, sep = "", stringsAsFactors = TRUE, col.names = c("mpg", "cylinders", "displacement", "horsepower", "weight", "accelertation", "model_year", "origin", "car_name") )
autompg[1:20,]

# Summarize the mpg variable
sum.autompg <- summary(autompg$mpg)
sum.autompg

# Calculate proportion of mpg above 25
prop.autompg <- sum(autompg$mpg > "25", na.rm = TRUE)/length(autompg$mpg)
prop.autompg

# Bar chart of variable mpg
barplot(autompg$mpg,
        ylim = c(0,50),
        main = "Bar Graph of MPG",
        col="lightblue")
box(which = "plot",
    lty = "solid",
    col = "black")

# Make a table for counts for mpg and model_year
counts <- table(autompg$mpg, autompg$model_year, dnn=c("MPG","Model Year"))
counts
